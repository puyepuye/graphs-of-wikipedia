...
def drawgraph3(networkx_graph,path=[],notebook=True,output_filename='graph.html'):
        """
        ...
        for node,node_attrs in networkx_graph.nodes(data=True):
            if node == path[0]:
                pyvis_graph.add_node(node,node_attrs, color='#ff4d4d')
            elif node == path[-1]:
                pyvis_graph.add_node(node,node_attrs, color='#fffd8f')
            elif node in path:
        ...
        for source,target,edge_attrs in networkx_graph.edges(data=True):
            if target in path and source in path:
                #speical color for edgews in the path
